...
class AIService:
    ...
    async def generate_theory(self, topic: str, lesson_title: str) -> dict:
        ...
        system = (
            ...
            "НЕ представляйся и НЕ добавляй никаких вступлений. "
            "Отвечай ТОЛЬКО валидным JSON без какого-либо текста до или после него."
        )
...
{{"theory": "текст в markdown", "examples": [{{"title":"..","code":"..","explanation":".."}}]}}"""
    ...
    async def generate_task(self, topic: str, difficulty: str,
        ...
        system = (
            ...
            "НЕ представляйся и НЕ добавляй никаких вступлений. "
            "Отвечай ТОЛЬКО валидным JSON на русском языке без какого-либо текста до или после."
        )
